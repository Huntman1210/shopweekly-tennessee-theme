// Global JavaScript for ShopWeekly Tennessee Theme
...
function trapFocus(container, elementToFocus = container) {
  ...
  function handleKeydown(event) {
    ...
    // If tab forward and focused on last element, focus on first element
    if (event.target === last && !event.shiftKey) {
      event.preventDefault();
      first.focus();
    }

    // If tab backward and focused on first element, focus on last element
...
function removeTrapFocus(elementToFocus = null) {
  document.removeEventListener('focusout', trapFocus);
...
class CartItems extends HTMLElement {
  ...
  updateLiveRegions(line, itemCount) {
    if (this.currentItemCount === itemCount) {
      ...
      const quantityElement = document.getElementById(`Quantity-${line}`) || document.getElementById(`CartDrawer-Quantity-${line}`);
...
// Initialize Tennessee theme features
    ...
    .then(cart => {
      ...
      cartLinks.forEach(link => {
        if (link.textContent.includes('Cart')) {
          link.innerHTML = `🛒 Cart (${cart.item_count})`;
        }
      });
    })
